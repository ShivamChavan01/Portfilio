...
| 8 | **Jira CI/CD Status Panel** | **Recognition strip / compact entry** | "**Won the Atlassian ForgeQuest Hackathon**" is a strong signal, but there is no link ("present it as an Atlassian ForgeQuest **winner** without a URL"). A win without artifact belongs in a recognition/hackathon strip, not a case study. |
